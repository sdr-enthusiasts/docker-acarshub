...
| EFFECT-01 through EFFECT-04 | Extract custom hooks from page components. EFFECT-01 DONE `7835350b`: `LiveMapPage.tsx` (688 lines, 12 useEffects) split into 5 domain hooks under `hooks/` — `useMapLifecycle.ts`, `useMapPauseState.ts`, `useMapZoomFreeze.ts`, `useMapFollowedAircraft.ts`, `useMapSidebarLayout.ts`. Page shrinks to ~275 lines. EFFECT-02 DONE `d3c8868f`: `LiveMessagesPage.tsx` (820 lines, 9 useEffects + 1 useLayoutEffect) split into 4 hooks — `usePageRegistration.ts` (new, generic/reusable — a candidate for AboutPage/AlertsPage/StatsPage/StatusPage in a future pass), `useMessageListHeight.ts`, `useMessageScrollAnchor.ts`, `useMessageFilters.ts` (also relocates the `globalFilterProps`/`getMessageFilterProps()` singleton Navigation.tsx reads — Navigation now imports it from the hook module instead of the page). Page shrinks to ~500 lines. EFFECT-03 DONE `202b761c`: `SearchPage.tsx` (1065 lines, 6 useEffects + 1 useLayoutEffect) split into 4 hooks — `useSearchStatePersistence.ts` (sessionStorage persistence across in-app navigation + `loadPersistedSearchState()` for the lazy useState initializers), `useSearchFormCollapse.ts` (scroll-driven auto-collapse, also owns `expandForm()`), `useSearchResultsSubscription.ts` (socket listener), `useSearchScrollMargin.ts` (virtualizer scroll-margin measurement, shares `appContentRef` with the form-collapse hook). Left inline: the debounce-timer cleanup (3 lines) and the page-registration effect (its `[setActivePageName, databaseSize]` deps don't match the generic `usePageRegistration` contract — forcing it through would silently change re-registration frequency). None of EFFECT-01/02/03 needed new dedicated hook test files — pure code relocation, existing page test suites (33 + 19 + 46 tests) already exercise every hook end-to-end; `hooks/**` full-suite coverage sits at 89.79% stmt / 70.55% branch, above the 88/88/55/88 threshold. EFFECT-04 (`AlertsPage.tsx`, 5 effects) remains open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| NIT-10                      | `acarshub-react/src/components/LogsViewer.tsx` test coverage — deferred during Phase 6 TEST-GAP-FE "everything else" pass. LogsViewer is a heavyweight virtualized component with its own state machine (log-level filtering, pause/resume scroll-follow, search/highlight, export, source-filter persistence). Worth a dedicated test bundle on the order of 30–60 tests. Out of scope for the original Phase 6 audit but should be picked up before the next major release. Suggested approach: mock the underlying log-source service and virtualization library; assert on filter/search behaviour and scroll-follow state transitions rather than DOM measurements.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
