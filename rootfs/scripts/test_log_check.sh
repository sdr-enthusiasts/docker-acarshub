...
for i in {1..1000}; do echo "acars $i" >> /run/acars/acars.json; done

# Prepare spoofed /run/acars/vdlm.json
for i in {1..1000}; do echo "vdlm $i" >> /run/acars/vdlm.json; done
...
head -1 /run/acars/acars.json | grep 'acars 501'
head -1 /run/acars/vdlm.json | grep 'vdlm 501'
tail -1 /run/acars/acars.json | grep 'acars 1000'
tail -1 /run/acars/vdlm.json | grep 'vdlm 1000'
