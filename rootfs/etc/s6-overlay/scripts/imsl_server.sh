...
if chk_enabled "${ENABLE_IMSL}"; then
    ...
    if true || [[ $((MIN_LOG_LEVEL)) -ge 4 ]]; then
        ...
        echo "Starting service" | stdbuf -oL awk '{print "[imsl_server] " strftime("%Y/%m/%d %H:%M:%S", systime()) " " $0}'
    fi
    ...
    # shellcheck disable=SC2016
    {
        ...
        socat -T 60 -u udp-listen:5557,fork,reuseaddr stdout | {
            ...
            ncat -4 --keep-open --listen 0.0.0.0 15557 | {
            cat
            kill -s INT 0
        }
    } 2>&1 | stdbuf -oL awk '{print "[imsl_server] " strftime("%Y/%m/%d %H:%M:%S", systime()) " " $0}'
