...
if chk_enabled "${ENABLE_HFDL}"; then
    ...
    # Listen for the output of dumphfdl (UDP), and make it available for multiple processes at TCP port 15556
    # shellcheck disable=SC2016
    {
        socat -T 60 -u udp-listen:5556,fork,reuseaddr stdout | {
            cat
            kill -s INT 0
        } |
            ncat -4 --keep-open --listen 0.0.0.0 15556 | {
            cat
            kill -s INT 0
        }
