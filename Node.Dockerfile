...
# ---------------------------------------------------------------
# rootfs layout:
#   COPY rootfs/       installs the base s6/nginx/script tree
#   COPY rootfs-node/  overlays Node.js-specific overrides:
#     • webapp.sh          – starts "node dist/server.js" instead of gunicorn
#     • 02-database.sh     – no-op (Drizzle migrations run inside the backend)
#     • nginx sites-enabled/acarshub – proxy_pass to port 8080 (Fastify)
#     • healthcheck.sh     – checks "node" process instead of "python3"
# ---------------------------------------------------------------
COPY rootfs/      /
COPY rootfs-node/ /
